...
### Added
- `cert-tofu` and `cert-fingerprints` RDP connection parameters to the parameter whitelist (`modify_connection()` and `create_connection()` now accept them), enabling RDP certificate Trust-on-First-Use and certificate fingerprint pinning

### Changed
- Updated `ignore-cert` parameter description to the protocol-neutral text from the Guacamole authentication-and-security docs (was previously Kubernetes-specific)
