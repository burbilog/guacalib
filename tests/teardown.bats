...
cleanup_timestamped_entries() {
    ...
    guacaman --config "$TEST_CONFIG" conngroup list >/dev/null 2>&1 | grep -E "test_[a-zA-Z0-9_]+_[0-9]{10}:" | cut -d: -f1 | while read -r group; do
    ...
    guacaman --config "$TEST_CONFIG" conn list >/dev/null 2>&1 | grep -E "  test_[a-zA-Z0-9_]+_[0-9]{10}:" | cut -d: -f1 | tr -d ' ' | while read -r conn; do
        [ -n "$conn" ] && guacaman --config "$TEST_CONFIG" conn del --name "$conn" >/dev/null 2>&1 || true
    done

    # Clean up test users with timestamp patterns
    guacaman --config "$TEST_CONFIG" user list >/dev/null 2>&1 | grep -E "test_[a-zA-Z0-9_]+_[0-9]{10}:" | cut -d: -f1 | while read -r user; do
        [ -n "$user" ] && guacaman --config "$TEST_CONFIG" user del --name "$user" >/dev/null 2>&1 || true
    done

    # Clean up test user groups with timestamp patterns
    guacaman --config "$TEST_CONFIG" usergroup list >/dev/null 2>&1 | grep -E "test_[a-zA-Z0-9_]+_[0-9]{10}:" | cut -d: -f1 | while read -r group; do
