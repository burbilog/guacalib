...
cleanup_timestamped_entries() {
    ...
    guacaman --config "$CONFIG_FILE" conngroup list 2>/dev/null | grep -E "test_[a-zA-Z_]+_[0-9]{10}:" | cut -d: -f1 | while read -r group; do
        if [ -n "$group" ]; then
    ...
    guacaman --config "$CONFIG_FILE" conn list 2>/dev/null | grep -E "  test_[a-zA-Z_]+_[0-9]{10}:" | cut -d: -f1 | tr -d ' ' | while read -r conn; do
        if [ -n "$conn" ]; then
