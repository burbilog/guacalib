...
- [x] **6. Глубокая вложенность SQL-запросов при удалении** (НЕ ПРИМЕНИМО)
  ...
  **Обоснование:** Guacamole - это сторонний продукт со своей схемой БД. Мы не можем модифицировать схему (добавлять CASCADE DELETE, stored procedures или триггеры), так как это нарушит совместимость с upstream. Текущий код функционален и корректен - транзакционность гарантирует атомарность операции.
...
- [x] **15. Неточный return type** (НЕ ТРЕБУЕТ ИСПРАВЛЕНИЯ)
  ...
  **Обоснование:** `NoReturn` является правильным типом. Функция `main()` всегда завершается через `sys.exit()`, который выбрасывает исключение `SystemExit`. Это означает, что функция никогда не возвращается нормально (never returns). `sys.exit(0)` - это не "нормальный возврат", а выброс исключения с кодом успеха. Аннотация `NoReturn` корректно отражает поведение функции.
...
- [x] **20. Unit-тесты для Python кода** (НЕ ТРЕБУЕТСЯ)

  **Обоснование:** Код проекта — почти исключительно CRUD операции с БД. Unit тесты с mock базы превращаются в проверку "вызывается ли execute с такими аргументами", что даёт мало ценности. Существующие bats integration тесты покрывают реальные сценарии с живой MySQL базой, что эффективнее ловит баги в SQL и данных. Integration тесты в данном случае ценнее чем unit тесты.
- [x] **21. Pre-commit hooks** (НЕ ТРЕБУЕТСЯ)

  **Обоснование:** Проект разрабатывается одним человеком с помощью AI ассистентов, которые уже выдают отформатированный код. Makefile содержит `make format` и `make format-check` для ручного использования. Pre-commit hooks полезны для больших команд с разными стилями кодирования, но для solo-проекта это unnecessary overhead.
- [x] **22. Type checking с mypy** в CI pipeline (НЕ ТРЕБУЕТСЯ)

  **Обоснование:** Type hints уже добавлены (п.25) и дают основную ценность — документацию и IDE support. Для solo-проекта с AI ассистентами enforcing mypy в CI — это overhead. Mypy можно запускать локально при необходимости, но bats тесты эффективнее ловят реальные баги.
- [x] **23. Документация API** с Sphinx или pdoc (НЕ ТРЕБУЕТСЯ)

  **Обоснование:** README.md уже содержит comprehensive examples для всех операций (~800 строк). Docstrings есть во всех методах, type hints добавлены. Для небольшой библиотеки это достаточная документация. Sphinx/pdoc — overkill, добавит maintenance overhead без реальной выгоды для практического использования.
...
**Отступления:**
- [x] **24. Длинные функции** (ПРИЕМЛЕМО)

  **Обоснование:** CLI handlers по своей природе длинные — они обрабатывают множество subcommands с разными options. Функция `handle_conngroup_command` (~300 строк) логически группирует связанные операции. Разбивка на мелкие функции усложнит flow без реальной выгоды. Код работает, протестирован, поддерживаем.
